...
plot_probit <- function(results1, titulo ="Plot Modelo",
  ...
  prepare_model_data <- function(results, color, extended_dosis) {
    ...
    data.frame(
      ...
      model = model_name[which(colors == color)],
      color = color,
      ld50 = ld50,
      ci_lower = ci_ld50[1],
      ci_upper = ci_ld50[2]
  ...
  all_doses <- c(results1$CorrectedData$dosis)
  ...
  plot_data1 <- prepare_model_data(results1, colors, extended_dosis)
  plot_data <- plot_data1

  # Combinar datos experimentales
  exp_data <- cbind(results1$CorrectedData, model = model_name)
